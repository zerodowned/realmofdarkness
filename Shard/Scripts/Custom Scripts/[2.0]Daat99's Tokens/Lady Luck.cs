...
namespace Server.Mobiles
{
	...
	public class LadyLuck : Mobile
	{
		...
		public class LadyLuckExchangeGump : Gump
		{
			...
			private int iTokensExchange, iCost;
				...
				if ( info.ButtonID == 3 )
				{
					...
					if(tr_Exchange != null)
					{
						...
						else
						{
							...
							{
								if ( tl.Owner == m_From.Serial && tl.Tokens >= iTokensExchange )
								{
									if (!tl.Deleted)
									{
										tl.Tokens -= iTokensExchange;
									...
									{
										m_From.PlaySound(1069); //play Hey!! sound
										m_From.SendMessage(32, "Hey, don't try to rob the bank!!!");
										m_From.CloseGump( typeof( LadyLuckExchangeGump ) );
									}
									return;
								}
